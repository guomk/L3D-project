...
import torch
import scipy
from lib.jax import raw_utils
from lib.jax import utils
...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True, load_depths=False):
    ...
    print('poses', poses.shape, 'bds', bds.shape)
    ...
    # factor=1 # FIXME - this is a hack to get the code to run
    ...
    sh = tuple([int(s) // factor for s in sh])
    print("HERE", poses.shape)
    if poses.shape[1] == 4:
    ...
    split = utils.DataSplit.TRAIN
    image_names = None
    exposure_percentile = 97. # TODO move to config
...
def load_raw_data(basedir, factor=8, width=None, height=None,
